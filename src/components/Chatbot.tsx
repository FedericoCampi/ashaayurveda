  ...
  return (
    <div className={`fixed bottom-4 right-4 ${isDarkMode ? 'dark' : ''}`}>
      <button
        ...
        } items-center justify-center w-14 h-14 rounded-full bg-green-500 text-white shadow-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition-all duration-300 ease-in-out`}
      >
      ...
      >
        <div className="flex justify-between items-center p-4 bg-green-500 text-white">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={toggleDarkMode} className="p-1 rounded-full hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400">
              {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>
            <button onClick={toggleChat} className="p-1 rounded-full hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 p-4 bg-gray-100 dark:bg-gray-800 overflow-y-auto">
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.isBot ? 'justify-start' : 'justify-end'} mb-4`}>
              <div className={`max-w-[75%] rounded-lg p-3 ${message.isBot ? 'bg-white dark:bg-gray-700 text-gray-800 dark:text-white' : 'bg-green-500 text-white'}`}>
                {message.text}
              </div>
            </div>
          ))}
        ...
        <form onSubmit={handleSendMessage} className="p-4 bg-white dark:bg-gray-900 border-t dark:border-gray-700 flex">
          <input
            ...
            className="flex-1 p-2 mr-2 rounded-md border dark:border-gray-700 focus:outline-none focus:border-green-500 dark:bg-gray-800 dark:text-white"
          />
          <button type="submit" className="p-2 bg-green-500 text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50">
